document.addEventListener("DOMContentLoaded", function () {
  // ==========================================================
  // PHASE 1: Lädt SOFORT (Core, UI, Plugins ohne SplitText)
  // ==========================================================
  initCacheFix();
  initGSAP();
  initLenis();
  ...
  // Komponenten & Logik, die keine Schriften brauchen
  initVJSPlayer();
  initMatterJS();
  initServiceSwiper();
  initAccordion();
  ...
  // ==========================================================
  // PHASE 2: Lädt ERST nach Font-Load (Alles mit SplitText)
  // ==========================================================
...
function initCursor() {
  ...
  let globalXTo = gsap.quickTo(".cursor", "x", {
    duration: 0.6,
    ease: "power3",
  });
  let globalYTo = gsap.quickTo(".cursor", "y", {
    duration: 0.6,
    ease: "power3",
  });
...
function initHeroAnimation() {
  ...
  if (
    ...
    if (!sessionStorage.getItem("visited")) {
      ...
      heroAnimation.from(
        ".g-navbar_wrapper",
        { duration: 1.25, y: -100, ease: "quint.out" },
        "<-0.5"
      );
      heroAnimation.from(
        ".hero_headline .word",
...
function initProjectAnimation() {
  ...
  projectHero
    ...
    .from(
      ...
      { y: 20, opacity: 0, ease: "expo.out", duration: 1, stagger: 0.1 },
      "<"
    )
    .from(
      ".work_project_description",
      { y: 20, duration: 1.5, opacity: 0, ease: "expo.out" },
      ">-0.75"
    );
}
...
function initWorkScrollIn() {
  ...
  workHomeScrollIn.from(
    ".work_container",
    { y: 50, opacity: 0, ease: "expo.out", duration: 1.5 },
    "< 0.2"
  );
}
  ...
  if (typeof $ !== "undefined") {
    $(".work_item_container").each(function () {
      ...
      workItemTimeline.to(
        ...
        { opacity: 1, duration: 0.2, ease: "quint.inOut" },
        "<0.4"
      );
    ...
    if (isReadyForScrollIndicator && event.deltaY > 0) {
      ...
      if (scrollProgress >= 100) {
        ...
        if (linkElement && linkElement.getAttribute("href")) {
          if (window.playPageTransition)
            window.playPageTransition(linkElement.getAttribute("href"));
        }
      }
    }
  });
...
function initWorkTitleList() {
  ...
  function updateActiveItems() {
    ...
    const items = Array.from(
      document.querySelectorAll(".work-title_list_item")
    );
    ...
    if (closestItem) {
      ...
      videos.forEach((video) => {
        video.classList.toggle(
          "active",
          video.getAttribute("data-work-scroll-video-name") === listName
        );
      });
    } else {
...
function initCmsFooter() {
  ...
  const currentProject = projects.find(
    (p) => p.getAttribute("data-link") === currentSlug
  );
  ...
  if (noticeLink && nameElement && imgWrapper) {
    ...
    }/work/${nextProject.getAttribute("data-link")}`;
...
function initCmsMoreProjects() {
  ...
  const replacementSlug = allProjects
    .map((p) => p.getAttribute("data-link"))
    .find((s) => !visibleSlugs.includes(s) && s !== currentSlug);
        ...
        if (firstItem) {
          ...
          setTimeout(() => {
            ...
            document.querySelectorAll(".work_video_item").forEach((video) => {
              video.classList.toggle(
                "active",
                video.getAttribute("data-work-scroll-video-name") ===
                  firstItemName
              );
            });
          }, 10);
        ...
        activeItems.slice(1).forEach((item, index) => {
          timeoutIds.push(
            setTimeout(() => {
              ...
              if (
                index === activeItems.length - 2 &&
                window.updateActiveItemsExport
              ) {
...
function initVJSPlayer() {
  ...
  document.querySelectorAll(".vjs-player").forEach((vjsPlayer) => {
    ...
    if (wrapper.getAttribute("data-vjs-initialized") === "true") {
      return;
    }
    ...
    if (vjsVideoLink) {
      videoElement.setAttribute(
        "src",
        `https://dl.dropboxusercontent.com/scl/fi/${vjsVideoLink}`
      );
    }
    ...
    wrapper.setAttribute("data-vjs-initialized", "true");

    wrapper.addEventListener("touchstart", () => {
      player.userActive(true);
      wrapper.classList.add("vjs-touch-active");
      setTimeout(() => wrapper.classList.remove("vjs-touch-active"), 200);
    });

    player.on("useractive", () => {
      wrapper.classList.add("vjs-user-active");
      wrapper.classList.remove("vjs-user-inactive");
      if ("ontouchstart" in window) player.reportUserActivity();
    });
...
function initEllipseCarousel() {
  ...
  document.addEventListener("mouseleave", (e) => {
    if (
      ...
      !e.target.closest(".people-card_item") &&
      lastHoveredItem
    ) {
      items.forEach((c) => c.classList.remove("active"));
...
function initImageTrail() {
  ...
  function fadeOut(img) {
    ...
    gsap.to(img, {
      ...
      duration: 0.8,
      ease: "expo.out",
      onComplete: () => gsap.set(img, { autoAlpha: 0 }),
    });
  }
  ...
  function handleMove(e) {
    ...
    if (
      Math.hypot(relX - state.last.x, relY - state.last.y) >
      ...
      fadeOut(
        state.trailImages[(state.globalIndex - 5) % state.trailImages.length]
      );
      state.globalIndex++;
    }
  }
  ...
  function cleanup() {
    ...
    for (const [img, ts] of state.trailImageTimestamps.entries()) {
      if (now - ts > 300) {
        fadeOut(img);
        state.trailImageTimestamps.delete(img);
      }
    }
  }
  ...
  const start = () => {
    if (!state.isActive) {
      ...
      state.trailInterval = setInterval(cleanup, 100);
    }
  };
  const stop = () => {
    if (state.isActive) {
      ...
      state.trailImages.forEach(fadeOut);
  ...
  window.addEventListener("resize", () => {
    window.innerWidth < 992 && state.isActive
      ? stop()
      : window.innerWidth >= 992 && !state.isActive && start();
  });
}
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      ...
      const walls = [
        ...
        Bodies.rectangle(
          canvas.offsetWidth + canvas.offsetWidth / 4,
          canvas.offsetHeight / 2,
          canvas.offsetWidth / 2,
          canvas.offsetHeight,
          { isStatic: true, render: { visible: false } }
        ...
        Bodies.rectangle(
          canvas.offsetWidth / 2,
          canvas.offsetHeight + canvas.offsetWidth / 4,
          canvas.offsetWidth,
          canvas.offsetWidth / 2,
          { isStatic: true, render: { visible: false } }
        ),
...
function initServiceHome() {
  ...
  if (window.splitTextInstances)
    window.splitTextInstances.forEach((i) => i.revert());
  ...
  containers.forEach((c) => {
    ...
    if (t) {
      c.titleSplit = new SplitText(t, {
        type: "words,chars",
        charsClass: "char",
        wordsClass: "word",
      });
      window.splitTextInstances.push(c.titleSplit);
    }
    if (p) {
      c.paraSplit = new SplitText(p, {
        type: "words,lines",
        linesClass: "split-line",
        wordsClass: "word",
        charsClass: "char",
      });
      window.splitTextInstances.push(c.paraSplit);
    }
  });
  ...
  containers.forEach((c, i) => {
    ...
    tl.set(c, {
      ...
      onStart: () => {
        ...
        if (i > 0) {
          for (let j = 0; j < i; j++) gsap.set(containers[j], { autoAlpha: 0 });
        }
      },
    });
    if (chars.length)
    ...
    if (i < containers.length - 1 && (chars.length || lines.length)) {
      tl.to(
        [...chars, ...lines],
        { autoAlpha: 0, y: -20, stagger: 0.02, duration: 0.4 },
        `+=0.3`
      );
      ...
      accordion.addEventListener("click", (e) => {
        ...
        if (
          ...
          accordion
            ...
            .forEach((sib) => {
              if (sib !== item)
                sib.setAttribute("data-accordion-status", "not-active");
            });
        }
  ...
  if (btn) {
    ...
    const update = () => {
      items.forEach((item, i) =>
        item.classList.toggle("visible", expanded || i < 6)
      );
      btn.querySelector(".button-text").textContent = expanded
        ? "Weniger anzeigen"
        : "Mehr anzeigen";
    };
    btn.addEventListener("click", () => {
      expanded = !expanded;
      update();
      [50, 300].forEach((d) => setTimeout(refreshLayout, d));
    });
    update();
  }

  if (document.querySelectorAll('[data-accordion-status="active"]').length > 0)
    [50, 300].forEach((d) => setTimeout(refreshLayout, d));
}
...
function initGalleryScroll() {
  ...
  medias.forEach((media) =>
    gsap.set(media, {
      ...
      y: (Math.random() - 0.5) * 0.05 * window.innerWidth,
    })
  );
